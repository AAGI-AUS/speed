...
For advanced and specialised experimental designs (e.g., split-split plot, strip-plot, incomplete block, p-rep, and more), see the companion vignette: [Complex Agricultural Experimental Designs with speed](complex_designs.html).
...
![](figures/crd-plot1-1.png){fig-align="default"}
...
The printed output from the returned design object gives a quick identity check: the final optimisation score (lower is better), how many iterations were run and whether it stopped early, the treatments, and the seed. For a fuller, statistically meaningful evaluation of the design - layout, replication, score decomposition, connectedness and more - use `summary()` (see [Evaluating a Design](#evaluating-a-design)). The output object contains some additional components, which can be seen below:
...
##   .. .. .. .. ..- attr(*, ".Environment")=<environment: 0x0000022516ab6008> 
...
![](figures/crd-plot2-1.png){fig-align="default"}

A nicely randomised[^1] and spatially optimal design!

[^1]: Note that this design is not a true randomised design, as `speed` performs restricted randomisation and this design has been optimised to reduce neighbour effects. However, it can still be considered a Completely Randomised Design as it does not account for any blocking or other sources of variation.
...
- **Evaluation** -- design-quality diagnostics:
  - **Connectedness** -- whether treatment effects are estimable. Fits `lm(~ <spatial factors + block> + treatment)` and checks for aliased treatment terms; the nuisance side of the model adjusts to whatever the design is stratified by (row/col, and a block factor when present).
  - **Concurrence** -- for *incomplete* block designs, how often pairs of treatments share a block (constant concurrences indicate a balanced incomplete-block design). It is skipped for complete designs such as RCBD and split-plots, where it carries no information.
...
The returned summary is also a list, so individual metrics can be pulled out programmatically:
...
![](figures/rcbd-plot1-1.png){fig-align="default"}
...
##   .. .. ..$ swap            : chr "treatment"
##   .. .. ..$ spatial_factors :Class 'formula'  language ~row + col
##   .. .. .. .. ..- attr(*, ".Environment")=<environment: 0x0000022514d79858> 
##   .. .. ..$ spatial_cols    : chr [1:2] "row" "col"
##   .. .. ..$ adj_weight      : num 1
##   .. .. ..$ bal_weight      : num 1
##   .. .. ..$ iterations      : num 10000
...
![](figures/rcbd-plot2-1.png){fig-align="default"}
...
![](figures/latin-square-plot1-1.png){fig-align="default"}
...
##   .. .. ..$ swap            : chr "treatment"
##   .. .. ..$ spatial_factors :Class 'formula'  language ~row + col
##   .. .. .. .. ..- attr(*, ".Environment")=<environment: 0x000002251027ab60> 
##   .. .. ..$ spatial_cols    : chr [1:2] "row" "col"
##   .. .. ..$ adj_weight      : num 1
##   .. .. ..$ bal_weight      : num 1
##   .. .. ..$ iterations      : num 10000
...
![](figures/latin-square-plot2-1.png){fig-align="default"}
...
![](figures/split-plot-plot1-1.png){fig-align="default"}

![](figures/split-plot-plot1-2.png){fig-align="default"}
...
::: {.callout-note}
`swap_all = TRUE` is set on the whole-plot level so that all subplots sharing a whole plot move together; the subplot level keeps the default single-cell swap.
:::
...
##   .. .. ..$ spatial_factors :Class 'formula'  language ~row + col
##   .. .. .. .. ..- attr(*, ".Environment")=<environment: 0x000002251e8576d0> 
##   .. .. ..$ spatial_cols    : chr [1:2] "row" "col"
##   .. .. ..$ adj_weight      : num 1
##   .. .. ..$ bal_weight      : num 1
##   .. .. ..$ iterations      : num 10000
...
![](figures/split-plot-plot2-1.png){fig-align="default"}
...
![](figures/split-plot-plot2-2.png){fig-align="default"}
...
Optimising without `linked_cols` moves `treatment` but leaves `variety_name` sitting in its original row order, so the pairing between the two is lost:
...
Naming the column in `linked_cols` carries it along with the treatment it belongs to:
...
Linked columns take no part in scoring, so adding them cannot change the design that is found. The two runs above produce exactly the same layout:

```{r linked-neutral}
identical(unlinked$design_df$treatment, linked$design_df$treatment)
```

They are also returned in their original position and with their original type - no extra columns appear in the output.
...
```{r linked-hierarchical-alt}
#| eval: false
speed(split_plot_linked, optimise = optimise,
      linked_cols = list(wp = "irrigation", sp = "variety"), seed = 42)
```

Since the hierarchy is already described level by level, `linked_cols` can equally go inside `optimise` alongside the other per-level arguments, which keeps each level's description in one place. Where a level sets `linked_cols` itself, that wins; the argument fills in the levels that do not.

```{r linked-hierarchical}
...
::: {.callout-note}
A bare character vector applies to every level, which in a hierarchical design is only valid when all levels swap the same column - a multi-environment trial, for example. Where levels swap different columns, name the level, since a column can only travel with one `swap` column.
:::
...
- [Complex Agricultural Experimental Designs with speed](complex_designs.html): Advanced and specialised designs, including split-split plot, strip-plot, incomplete block, p-rep, and more.
