...
- **Keep `@param` entries short** - a sentence or two saying what the
  argument is, its default, and for hierarchical arguments “For
  hierarchical designs, can be a named list with names matching `swap`.
  See details for more information.” Anything longer - how the argument
  interacts with other arguments, worked explanations, edge cases -
  belongs in `@details` or the vignette, not in the parameter list.
